;(function($){
    ...
    function _open(cfg,typ,context){
        ...
        if(surl.indexOf("?")!==-1){
            if(surl.indexOf("ofoShare=")===-1){
                surl+="&ofoShare="+st;
            }else{
                surl = surl.replace(/ofoShare=[^&]*(&?)/,"ofoShare="+st+"$1");
            }
        }else{  
            surl+="?ofoShare="+st;
        }
        ...
        if(typ==="weixin"){
            window.open("http://ofoyou.com/geshui_source/wcode.html?url="+surl+"&title="+stitle+"&desc="+sdesc,"_blank","width= 620,height= 700,top= 100, left= 200, toolbar=no, menubar= no, scrollbars= no, resizable= yes, location= no, status=no ");
        }
